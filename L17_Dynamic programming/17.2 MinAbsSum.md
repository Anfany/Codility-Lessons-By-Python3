...
利用动态规划的思想，[参见](https://mp.weixin.qq.com/s/CxJ_bB-1y3AxBVReO2YK5A)。假设达到N-2，可以获得的最小值为Min(N-2)，那么最终的最小值Min(N-1)就为|Min(N-2)+(-1)\*A[N-1]|和|Min(N-2)+1\*A[N-1]|中的较小值，以此类推，Min(N-2)的最小值就为|Min(N-3)+(-1)\*A[N-2]|和|Min(N-3)+1\*A[N-3]|中的较小值。
